...
#include "behavior_tree_logger/BT_logger_generated.h"
...
int main(int argc, char* argv[])
{
    ...
    auto printStatus = [](BT_Serialization::Status status)
    {
        switch (status)
        {
        case BT_Serialization::Status::SUCCESS: return ( "\x1b[32m"  "SUCCESS" "\x1b[0m"); // RED
        case BT_Serialization::Status::FAILURE: return ( "\x1b[31m"  "FAILURE" "\x1b[0m"); // GREEN
        case BT_Serialization::Status::RUNNING: return ( "\x1b[33m"  "RUNNING" "\x1b[0m"); // YELLOW
        case BT_Serialization::Status::IDLE:    return ( "\x1b[36m" "IDLE   " "\x1b[0m"); // CYAN
        }
        return "Undefined";
    };
